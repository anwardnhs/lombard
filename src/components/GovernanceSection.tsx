...
const GovernanceSection = () => {
  ...
  const handleOpenModal = useCallback((member: BoardMember) => {
  ...
  return (
    <div className="bg-[#F4F4F0] font-poppins text-[#0F120F] min-h-screen">
      ...
      {/* Hero Section - Centered, Formal, Typographic */}
        ...
        <div className="container mx-auto max-w-[1100px] text-center">
          ...
          >
            ...
            <p className="text-lg sm:text-xl text-[#0F120F]/85 max-w-3xl mx-auto leading-relaxed font-semibold">
              Eleven distinguished professionals providing strategic oversight, risk stewardship, and governance across finance, law, technology, and public policy.
            </p>
          </motion.div>
        </div>
      </section>
          ...
          <main className="lg:col-span-9 space-y-24">
              ...
              {boardGroups.map((group, groupIdx) => (
                <div key={groupIdx} className="mb-20 last:mb-0">
                  ...
                  <div className="flex items-center gap-6 mb-10">
                    <h2 className="text-2xl sm:text-3xl font-serif text-[#0F120F]">
                      {group.title}
                    </h2>
                    ...
                    {group.members.map((member, i) => {
                      ...
                      return (
                        <motion.article
                          ...
                          className={`group bg-white border rounded-sm overflow-hidden flex flex-col transition-all duration-500 ${
                            isExecutive 
                            ? 'border-[#C7D3BC]/40 shadow-[0_4px_20px_rgba(199,211,188,0.15)] hover:shadow-[0_8px_32px_rgba(199,211,188,0.25)]'
                            : 'border-[#0F120F]/10 hover:border-[#C7D3BC]/30 hover:shadow-[0_4px_20px_rgba(0,0,0,0.08)]'
              ...
              <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center bg-white p-8 sm:p-10 lg:p-12 border border-[#0F120F]/10 rounded-sm shadow-[0_2px_16px_rgba(0,0,0,0.04)]">
                ...
                <div>
                  <p className="text-base text-[#0F120F]/80 leading-relaxed font-semibold mb-8">
                    The Board delegates focused oversight to specialist committees, ensuring rigorous governance, disciplined capital allocation, and robust risk stewardship across the Group. These committees sharpen accountability and accelerate decision-making.
                  </p>
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div 
                        key={idx} 
                        className="flex items-center gap-4 py-3 border-b border-[#0F120F]/5 last:border-0"
                      >
            ...
            <section ref={documentsSectionRef} id="documents" className="pt-12 border-t border-[#0F120F]/10">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                ...
                <a 
                  href="/investors" 
                  className="text-xs font-bold uppercase tracking-[0.15em] text-[#0F120F]/80 hover:text-[#C7D3BC] border-b border-[#0F120F]/20 hover:border-[#C7D3BC] pb-1 transition-all"
                >
                  Investor Relations →
                </a>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {documents.map((doc, i) => (
                  <a 
                    key={i} 
                    href={doc.href}
                    className="group flex items-center justify-between p-6 bg-white border border-[#0F120F]/10 hover:border-[#C7D3BC]/40 rounded-sm transition-all hover:shadow-[0_4px_16px_rgba(0,0,0,0.06)] focus:outline-none focus:ring-2 focus:ring-[#C7D3BC]"
                  >
                    <div className="flex items-center gap-5">
                      <div className="w-11 h-11 bg-[#F4F4F0] flex items-center justify-center text-[#0F120F]/50 group-hover:bg-[#C7D3BC] group-hover:text-[#0F120F] transition-all rounded-sm flex-shrink-0">
                        <FaFileContract className="w-5 h-5" />
                      </div>
                      <span className="text-sm sm:text-base font-semibold text-[#0F120F]">
                        {doc.title}
                      </span>
                    </div>
                    <FaArrowDown className="w-4 h-4 text-[#0F120F] group-hover:translate-x-1 transition-all flex-shrink-0" />
                  </a>
                ))}
              </div>
            </section>
      ...
      <AnimatePresence>
        {selectedMember && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#0F120F] backdrop-blur-md"
          ...
          >
            ...
            >
              ...
              <button 
                onClick={handleCloseModal}
                className="absolute top-5 right-5 z-10 w-10 h-10 rounded-full bg-white/10 hover:bg-[#0F120F] flex items-center justify-center text-[#0F120F] hover:text-white transition-all border border-[#0F120F]/10 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-[#C7D3BC]"
              ...
              <div className="flex-1 p-8 sm:p-10 lg:p-14 overflow-y-auto">
                <div className="mb-8 pb-6 border-b border-[#0F120F]">
                  <h3 className="text-3xl sm:text-4xl font-serif text-[#0F120F] mb-4 leading-tight">
                    {selectedMember.name}
                  </h3>
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#0F120F] mb-1">
                    {selectedMember.role}
                  </p>
                  {selectedMember.subRole && (
                    <p className="text-sm text-[#0F120F]/80 mt-2 italic font-semibold">
                      {selectedMember.subRole}
                    </p>
                  )}
                </div>
                ...
                <div className="prose prose-base max-w-none text-[#0F120F] leading-relaxed">
                  <p className="font-semibold">{selectedMember.bio}</p>
                </div>
              </div>
            </motion.div>
          </motion.div>
